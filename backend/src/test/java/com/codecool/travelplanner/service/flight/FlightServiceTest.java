...
class FlightServiceTest {
    ...
    @Test
    void getFlightOffersDelegatesToProvider() {
        FlightService service = new FlightService(flightDataProvider);

        LocalDate date = LocalDate.of(2026, 8, 27);
        List<FlightOfferDto> expectedDtos = List.of(new FlightOfferDto());

        when(flightDataProvider.getFlightOffers("BUD", "LON", date))
                .thenReturn(expectedDtos);

        List<FlightOfferDto> result =
                service.getFlightOffers("BUD", "LON", date);

        assertThat(result).isEqualTo(expectedDtos);
        verify(flightDataProvider)
                .getFlightOffers("BUD", "LON", date);
    }
}